...
- 호스트 관리 인증에서는 private token을 읽지 않고 검증된 `auth.json`만 호환 모드로 교체
- 프로필별 한도 조회는 무작위 `0700` 임시 `CODEX_HOME`과 `0600` 인증 파일에서만 수행하고, 갱신 인증을 다시 봉인한 뒤 평문을 best-effort 제거
- 30초 한도 조회와 계정 전환을 직렬화하고, 대상 교체 직후에는 회전형 refresh token을 강제로 다시 갱신하지 않음
...
사용자가 `Guided Switch`를 선택하면 macOS `System Events`를 통해 공식 앱 메뉴의 `Log Out`/`로그아웃` 항목만 실행합니다. 화면 내용, 키 입력, 브라우저 쿠키 또는 다른 앱의 UI는 읽지 않습니다. 최초 실행 시 macOS 자동화 권한을 요청할 수 있으며, 거부되면 로그아웃하지 않고 중단합니다.
