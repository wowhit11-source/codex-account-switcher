...
1. 공식 ChatGPT/Codex 앱에 로그인된 상태에서 `현재 계정 등록`을 누릅니다.
2. 앱은 현재 `auth.json`을 검증한 뒤 즉시 AES-GCM으로 암호화합니다.
3. `계정 추가`를 누르면 별도 임시 `CODEX_HOME`에서 공식 App Server의 `account/login/start` 브라우저 흐름을 시작합니다.
4. 브라우저에서 사용자가 직접 두 번째 계정의 로그인·MFA를 완료합니다.
...
5. 세션·히스토리·설정·skills의 경로, 크기, 수정 시각, SHA-256 스냅샷을 만듭니다.
6. 현재 인증 캐시를 최신화해 현재 프로필에 다시 암호화 저장합니다.
7. 실제 Bundle Identifier로 공식 앱에 정상 종료를 요청합니다.
8. 정상 종료가 15초 안에 끝나지 않으면 사용자의 별도 승인 없이는 강제 종료하지 않습니다.
9. 기존 인증을 암호화된 긴급복구 백업으로 저장한 뒤 대상 인증을 임시 파일·`fsync`·원자적 rename으로 교체합니다.
10. 권한 `0600`, JSON 구조, `account/read(refreshToken: true)`, 실제 이메일 일치를 검증합니다.
11. 공식 앱을 원래 설치 경로에서 다시 실행하고 보호 상태를 비교합니다.
...
Accessibility, 브라우저 쿠키, 화면 녹화, 비공개 Keychain 접근은 요구하지 않습니다.
...
- 공식 데스크톱 호스트가 향후 `chatgptAuthTokens` 같은 호스트 관리형 토큰만 사용하고 `auth.json`을 무시하면 원클릭 전환을 성공으로 표시해서는 안 됩니다. 이 경우 공식 로그아웃·로그인 기반 Guided Switch만 사용해야 합니다.
- 공식 App Server에서 별도 데스크톱 호스트의 내부 토큰 상태를 가로채는 인터페이스는 사용하지 않습니다. 앱 재실행 뒤의 최종 동일 세션 동작은 Session Continuity Test로 확인합니다.
