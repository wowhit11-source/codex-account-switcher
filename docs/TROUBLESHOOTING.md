...
`account/rateLimits/read`가 인증 모드 또는 서비스 상태 때문에 응답하지 않은 경우입니다. `계정 정보 새로고침`을 누르면 현재 `auth.json` 계정과 저장 프로필별 격리 조회를 다시 실행합니다. 프로필 아래에 `인증 갱신 필요`가 표시되면 오른쪽 메뉴에서 해당 계정 인증을 갱신하세요. 추정값은 표시하지 않으며 API-key-only 또는 Bedrock 인증에서는 ChatGPT 사용량이 없을 수 있습니다.
