...
1. 메뉴에서 비활성 프로필의 `전환`을 누릅니다. 호스트 관리 인증, `keyring`, 알 수 없는 저장소 또는 검증되지 않은 `auth.json` 환경에서는 버튼이 비활성화됩니다.
...
13. 공식 앱을 원래 설치 경로에서 다시 실행하고 host-managed 인증이 나타나지 않는지 다시 관찰한 뒤 보호 상태를 비교합니다.
...
- 공식 데스크톱 호스트가 `chatgptAuthTokens` 같은 호스트 관리형 토큰을 사용하거나 `auth.json`을 무시하면 원클릭 전환 버튼을 비활성화합니다. `Guided Switch`는 공식 앱을 열고 사용자가 앱의 계정 메뉴에서 직접 로그아웃·로그인하도록 안내합니다.
