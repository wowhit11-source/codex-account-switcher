...
- 공식 데스크톱 호스트가 `chatgptAuthTokens` 같은 호스트 관리형 토큰을 사용하는 환경에서도 검증된 `auth.json` 교체를 호환 모드로 허용합니다. 다만 별도 App Server 검증은 공식 데스크톱 호스트가 실제로 같은 계정을 수용했다는 증거가 아니므로 전환 후 공식 앱에서 계정을 확인해야 합니다.
